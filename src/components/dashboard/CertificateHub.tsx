...
import { Award, Download, Printer, RefreshCw, Search, ShieldCheck, Trash2, CheckCircle2, AlertCircle, Eye, ExternalLink, QrCode } from 'lucide-react';
...
export default function CertificateHub({ coursesList, studentList }: CertificateHubProps) {
  ...
  const [selectedBatchNumber, setSelectedBatchNumber] = useState<number>(10);
  const [fourDigitBatchCode, setFourDigitBatchCode] = useState<string>('0208'); // e.g. 0208 for DPT0208
  ...
  // Generate Certificates for filtered batch
  const handleGenerateBatchCertificates = async () => {
    ...
    if (filteredStudents.length === 0) {
      setMessage('⚠️ No students found in this course and batch.');
      setTimeout(() => setMessage(''), 4000);
      return;
    }

    const batchPrefix = `DPT${rawDigits}`;
    const yearDigits = new Date().getFullYear().toString().slice(-2); // e.g. '26'

    setGenerating(true);
    setMessage('Generating batch certificates and QR records...');

    try {
      const recordsToUpsert = filteredStudents.map((student, idx) => {
        ...
        return {
          ...
          course_name: selectedCourse?.name || 'Academic Course',
      ...
      setMessage(`✅ Successfully generated ${recordsToUpsert.length} certificate(s) for ${batchPrefix}!`);
  ...
  const handleDownloadQrPng = async (cert: CertificateRecord) => {
    try {
      ...
      ctx.fillText('OFFICIAL CERTIFICATE QR CODE', width / 2, 78);
      ...
      ctx.drawImage(qrCanvas, (width - 340) / 2, 100, 340, 340);
      ...
      ctx.fillRect((width - 320) / 2, 530, 320, 42);

      ctx.strokeStyle = '#cbd5e1';
      ctx.lineWidth = 1;
      ctx.strokeRect((width - 320) / 2, 530, 320, 42);

      ctx.fillStyle = '#0f172a';
      ctx.font = 'bold 18px monospace';
      ctx.fillText(cert.certificate_code, width / 2, 557);
  ...
  return (
    <div style={{ padding: '1rem 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>
              Generate official certificate records with embedded QR codes in format <code style={{ color: '#fbbf24' }}>DPT0208/26/001</code>.
            </p>
          </div>
        </div>
      ...
      {/* Generator Form Card */}
      <div className="glass-card" style={{ padding: '1.75rem', borderRadius: '16px', border: '1px solid rgba(217, 119, 6, 0.2)', background: 'linear-gradient(180deg, rgba(255,255,255,0.9) 0%, rgba(248,250,252,0.9) 100%)', marginBottom: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-main)' }}>
          <QrCode size={20} className="text-primary" /> Batch Certificate Generator
        </h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.25rem', marginBottom: '1.5rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <span style={{ background: '#e2e8f0', padding: '0.6rem 0.6rem', border: '1px solid #cbd5e1', borderRight: 'none', borderRadius: '8px 0 0 8px', fontSize: '0.85rem', fontWeight: 800, color: '#0f172a' }}>
                DPT
              </span>
              <input
                type="text"
            ...
            <span style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '0.2rem', display: 'block' }}>
              Generates prefix: <strong style={{ color: '#d97706' }}>DPT{fourDigitBatchCode || '0000'}</strong>
            </span>
          </div>

          <div>
            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '0.4rem' }}>Grade / Honor Description</label>
            <input
              type="text"
        ...
        <div style={{ background: '#f1f5f9', padding: '0.85rem 1.25rem', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.25rem', border: '1px solid #e2e8f0' }}>
          <div style={{ fontSize: '0.85rem', color: '#334155' }}>
            Target Students: <strong>{filteredStudents.length} student(s)</strong> in Batch {selectedBatchNumber}
          </div>
          <div style={{ fontSize: '0.85rem', color: '#0f172a' }}>
            Code Format Preview: <code style={{ background: '#e2e8f0', color: '#b45309', padding: '0.25rem 0.6rem', borderRadius: '6px', fontWeight: 800 }}>DPT{fourDigitBatchCode || 'XXXX'}/{new Date().getFullYear().toString().slice(-2)}/001</code>
          </div>
        </div>
        ...
        <button
          ...
          style={{ padding: '0.75rem 1.5rem', borderRadius: '10px', fontWeight: 800, background: 'linear-gradient(135deg, #d97706 0%, #b45309 100%)', border: 'none', color: 'white', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}
        >
          <Award size={18} /> {generating ? 'Generating Certificates...' : `Generate ${filteredStudents.length} Certificate Records`}
        </button>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                  ...
                  <th style={{ padding: '0.75rem 1rem' }}>Course Program</th>
                ...
                {filteredCertificates.map(cert => (
                  <tr key={cert.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '0.85rem 1rem' }}>
                      <code style={{ background: '#f1f5f9', color: '#b45309', padding: '0.2rem 0.5rem', borderRadius: '6px', fontWeight: 800, border: '1px solid #cbd5e1' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          ...
          <div style={{ background: '#ffffff', width: '100%', maxWidth: '850px', maxHeight: '90vh', overflowY: 'auto', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)', padding: '2rem' }}>
            ...
            <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid #e2e8f0', paddingBottom: '1rem' }}>
              <h4 style={{ margin: 0, fontWeight: 800, color: '#0f172a', fontSize: '1.1rem' }}>Official Certificate Preview</h4>
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button
                  onClick={() => window.print()}
                  className="btn btn-primary"
                  style={{ padding: '0.5rem 1.25rem', borderRadius: '8px', background: '#d97706', color: 'white', fontWeight: 700, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
                >
            ...
            <div className="printable-diploma" style={{ border: '12px double #d97706', padding: '3rem 2.5rem', textAlign: 'center', background: '#fffdfa', position: 'relative', color: '#0f172a', fontFamily: 'serif' }}>
              
              {/* Corner Ornaments */}
              <div style={{ position: 'absolute', top: '15px', left: '15px', fontSize: '1.5rem', color: '#d97706' }}>❖</div>
              <div style={{ position: 'absolute', top: '15px', right: '15px', fontSize: '1.5rem', color: '#d97706' }}>❖</div>
              <div style={{ position: 'absolute', bottom: '15px', left: '15px', fontSize: '1.5rem', color: '#d97706' }}>❖</div>
              <div style={{ position: 'absolute', bottom: '15px', right: '15px', fontSize: '1.5rem', color: '#d97706' }}>❖</div>
              ...
              <p style={{ fontFamily: 'sans-serif', fontSize: '0.85rem', color: '#b45309', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 800, margin: '0 0 2rem 0' }}>
              ...
              <h2 style={{ fontSize: '2.5rem', fontWeight: 900, color: '#0f172a', margin: '0 0 1rem 0', textDecoration: 'underline #f59e0b 2px', textUnderlineOffset: '8px' }}>
              ...
              <h3 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#b45309', margin: '0 0 0.4rem 0' }}>
                {activePreviewCert.course_name}
              </h3>
              <p style={{ fontFamily: 'sans-serif', fontSize: '0.9rem', color: '#64748b', fontWeight: 700, margin: '0 0 2rem 0' }}>
                Batch {activePreviewCert.batch_number} • {activePreviewCert.grade_description || 'Passed with Distinction'}
              </p>
